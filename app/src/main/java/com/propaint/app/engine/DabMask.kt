...
object DabMaskGenerator {
    ...
    /** バイナリダブ: AA なし硬さ無視、円内 255 / 円外 0 */
    fun createBinaryDab(cx: Float, cy: Float, diameter: Float): DabMask? {
        if (diameter < 0.2f) return null
        val r = diameter / 2f
        if (r < 1f) {
            val data = IntArray(9); data[4] = 255
            return DabMask(cx.toInt() - 1, cy.toInt() - 1, 3, data)
        }
        val d = ceil(diameter).toInt().let { if (it % 2 == 0) it + 1 else it }
        val rSq = r * r
        ...
        for (y in 0 until d) {
            ...
            for (x in 0 until d) {
                val dx = x - center
                data[ro + x] = if (dx * dx + dy * dy <= rSq) 255 else 0
            }
        }
        return DabMask((cx - d / 2f).toInt(), (cy - d / 2f).toInt(), d, data)
    }

    fun createDab(cx: Float, cy: Float, diameter: Float, hardness: Float): DabMask? {
        if (diameter < 0.2f) return null
        ...
        val rawD = ceil(diameter).toInt() + 2
