...
class PaintViewModel(application: Application) : AndroidViewModel(application) {
    ...
    /**
     * ボックスブラー: position 周辺 radius ピクセルをプリマルチプライド平均してアンプリマルチプライドで返す。
    ...
    private fun sampleBoxBlurAt(position: Offset, radius: Int): Color? {
        ...
        val avgA = a / count
        if (avgA < 0.001f) return null  // 全て透明なら色情報なし
        // プリマルチプライドの平均をアンプリマルチプライド
        val invA = 1f / avgA
        return Color(
            red   = (r / count * invA).coerceIn(0f, 1f),
            green = (g / count * invA).coerceIn(0f, 1f),
            blue  = (b / count * invA).coerceIn(0f, 1f),
            alpha = avgA,
        )
    }
